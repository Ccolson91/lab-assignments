...
const repeatMyParam = () => {
    axios.get('http://localhost:3000/repeat/whodat').then( res => {
...
const querySearch = () => {
    console.log('test')
    axios.get('http://localhost:3000/query-test?name=christian&age=22').then(res => {
        console.log(res.data)})
}

document.getElementById('query-button').addEventListener('click', querySearch)
...
/*
    ...
    1: Send no queries on the URL -- what happened? You get a message saying you sent an empty query

    2: Send more than 1 query on the URL -- what happened? You get a message saying you sent more than 1 query
*/
...
const createFood = (e) => {
    e.preventDefault()
    let foodInput = document.getElementById('text-field')
    let body = {
